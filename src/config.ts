// 設定を集約するスクリプト（モジュールにしない）
// deploy時に scripts/deploy.sh が生成する config.generated.js から流し込む
...
function getConfig(props: ScriptProperties): AppConfig {
  const spreadsheetId = extractId(props.getProperty('SPREADSHEET_ID') || BASE_CONFIG.spreadsheetId);
  const articlesSheet =
    props.getProperty('ARTICLES_SHEET_NAME') || BASE_CONFIG.sheets.articles;
  const slackSheet =
    props.getProperty('SLACK_SHEET_NAME') || BASE_CONFIG.sheets.slackMessages;
  const imagesFolder =
    props.getProperty('DRIVE_IMAGES_FOLDER') || BASE_CONFIG.drive.imagesFolderName;
