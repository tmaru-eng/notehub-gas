...
function openSheetOrThrow(name) {
...
function buildArticleLink(id) {
    const base = (CONFIG.webAppUrl && CONFIG.webAppUrl.replace(/\/$/, '')) ||
        (function () {
            try {
...
function getArticles() {
    try {
        ...
        return values.map((row) => {
            var _a, _b, _c, _d, _e;
            return ({
                id: String((_a = row[0]) !== null && _a !== void 0 ? _a : ''),
                title: String((_b = row[1]) !== null && _b !== void 0 ? _b : ''),
                content: String((_c = row[2]) !== null && _c !== void 0 ? _c : ''),
                createdAt: row[3] instanceof Date ? row[3].toISOString() : String((_d = row[3]) !== null && _d !== void 0 ? _d : ''),
                updatedAt: row[4] instanceof Date ? row[4].toISOString() : String((_e = row[4]) !== null && _e !== void 0 ? _e : ''),
                tags: (row[5] || '')
                    .toString()
                    .split(',')
                    .map((t) => t.trim())
                ...
                authorName: String(row[7] || guessNameFromEmail(row[6])),
            });
        });
    }
    catch (e) {
...
function updateArticle(articleData) {
    if (!(articleData === null || articleData === void 0 ? void 0 : articleData.id))
        return 'エラー: 更新対象のIDがありません。';
    try {
        const sheet = openSheetOrThrow(CONFIG.sheets.articles);
...
// ---- Slack 取得系 ----
function getSlackMessages(options = {}) {
    try {
        ...
        let messages = rows.map((row) => {
            ...
            return ({
                timestamp: String((_a = row[idx('Timestamp')]) !== null && _a !== void 0 ? _a : ''),
                user: {
                    id: String((_b = row[idx('UserID')]) !== null && _b !== void 0 ? _b : ''),
                    name: String((_c = row[idx('UserName')]) !== null && _c !== void 0 ? _c : ''),
                    avatar: String((_d = row[idx('UserAvatar')]) !== null && _d !== void 0 ? _d : ''),
                },
                text: String((_e = row[idx('Text')]) !== null && _e !== void 0 ? _e : ''),
                channel: {
                    id: String((_f = row[idx('ChannelID')]) !== null && _f !== void 0 ? _f : ''),
                    name: String((_g = row[idx('ChannelName')]) !== null && _g !== void 0 ? _g : ''),
                },
                link: String((_h = row[idx('MessageLink')]) !== null && _h !== void 0 ? _h : ''),
...
function fetchChannelAndAppend(sheet, token, channelId) {
    var _a;
    ...
    if (lastRow > 1) {
        ...
        if (oldestTs.startsWith("'"))
            oldestTs = oldestTs.substring(1);
    }
    ...
    if (messages.length === 1 && firstTrunc === oldestTrunc)
        return;
    const userCache = {};
    ...
    for (const msg of messages) {
        if (msg.subtype === 'channel_join' || msg.subtype === 'channel_leave')
            continue;
        if (typeof msg.text === 'string' && msg.text.indexOf(notificationMarker) !== -1)
            continue; // 自身の通知はスキップ
        const msgTrunc = Math.floor(parseFloat(msg.ts) * 100) / 100;
        if (msgTrunc === oldestTrunc)
            continue;
        const uid = msg.user;
            ...
            if (!userCache[uid]) {
                try {
                    ...
                    if (userJson.ok && userJson.user) {
                        ...
                        avatar = ((_a = userJson.user.profile) === null || _a === void 0 ? void 0 : _a.image_48) || '';
        ...
        try {
            Utilities.sleep(1200); // Rate limit 対策
            const plResp = UrlFetchApp.fetch(`https://slack.com/api/chat.getPermalink?channel=${channelId}&message_ts=${msg.ts}`, options);
...
function getContent() {
    ...
    const formattedSlack = slackMessages.map((msg) => ({
        type: 'slack',
        id: msg.id,
        title: '',
        content: msg.text,
...
function searchContent(query) {
    ...
    return all.filter((item) => {
        ...
        const slackText = item.content.toLowerCase().includes(q) ||
            item.tags.some((t) => t.toLowerCase().includes(q)) ||
            item.author.name.toLowerCase().includes(q) ||
            (((_a = item.channel) === null || _a === void 0 ? void 0 : _a.name) || '').toLowerCase().includes(q);
        return slackText;
    });
}
// ---- 画像アップロード ----
function uploadImageToDrive(originalFileName, mimeType, base64Data) {
    try {
        const ss = SpreadsheetApp.openById(CONFIG.spreadsheetId);
        ...
        try {
            ...
            Logger.log(`DOMAIN_WITH_LINK での共有設定に失敗: ${shareErr instanceof Error ? shareErr.message : String(shareErr)}`);
        }
        if (!sharingApplied) {
            try {
...
function notifySlackNewArticle(payload) {
